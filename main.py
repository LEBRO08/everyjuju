...
async def on_message(message):
    ...
    # Define probabilities for sending a message and adding a reaction
                ...
                add_reaction_probability = 0.3  # 30% chance to add a reaction

    # Decide whether to send a message
                if random.random() < send_message_probability:
                    # Typing for response
    ...
    # Decide whether to add a reaction
                if random.random() < add_reaction_probability:
                    try:
                        random_emoji = random.choice(emoji_options)
                        await asyncio.sleep(random.randint(5, 8))
                        await message.add_reaction(random_emoji)
    ...
    # Only joining raffle = or above 0.1$
        ...
        #            break
                    ...
                    #                await asyncio.sleep(random.randint(2, 4))
                    #                await message.channel.send(response)
                    # Exit the loop after processing not 0.01$ each or more
                    break
